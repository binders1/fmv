...
for(k in seq_len(nrow(ag_regions_key))) {
  ...
  ffr_dir <- file.path(ddir, "model/full/frr/restricted")
  
  pred_dir <- file.path(ffr_dir, "predictions")
  perform_dir <- file.path(ffr_dir, "performance")
  imp_dir <- file.path(ffr_dir, "importance")
  
  pred_file <- 
    file.path(
      pred_dir, paste0("pred_ffr_", k, ".pqt")
    )
  
  perform_file <-  
    file.path(
      perform_dir, paste0("stats_ffr_", k, ".pqt")
    )
  
  imp_file <-  
    file.path(
      imp_dir, paste0("import_ffr_", k, ".pqt")
    )
