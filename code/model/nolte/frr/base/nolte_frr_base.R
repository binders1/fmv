...
for(k in seq_len(nrow(ag_regions_key))) {
  ...
  ## Select counties in current FRR ####
  counties_to_include <- 
    ag_regions_ref %>%
    filter(id == k) %>%
    pull(fips) %>%
  ...
  df_import <- 
    ...
    select(sid, fips, log_priceadj_ha, 
  ...
  nfb_dir <- file.path(ddir, "model/nolte/frr/base")
  
  pred_dir <- file.path(nfb_dir, "predictions")
  perform_dir <- file.path(nfb_dir, "performance")
  imp_dir <- file.path(nfb_dir, "importance")
  
  pred_file <- 
    file.path(
      pred_dir, paste0("pred_nfb_", k, ".pqt")
    )
  
  perform_file <-  
    file.path(
      perform_dir, paste0("stats_nfb_", k, ".pqt")
    )
  
  imp_file <-  
    file.path(
      imp_dir, paste0("import_nfb_", k, ".pqt")
    )
