...
\Large
 \begin{center}
Accounting for regional market dynamics improves and extends land value estimates\footnote[2]{Gold and Binder acknowledge financial support from the David \& Lissa Leege Endowment and professional development grant funds from the Faculty Life Committee of St Olaf College. They would also like to thank Rushikesh Jadhav for research assistance and Sara Dale and Craig Rice for their indispensable technical assistance.}\\ 
...
Specifying nine farm resource region-wide models enables the inclusion of observations that are dropped in county-level modeling. 
...
\textit{Exploration of accuracy-improvement mechanisms}
\begin{itemize}
    \item We see that the models with our expanded predictor set improve performance. The mechanism seems relatively straightforward: our new predictors add new valuable information to the RF model, allowing it to bin observations more accurately.
    \item We see that the FRR models provide substantial \textit{further} improvement. The mechanism here seems less obvious, but my theory is that is can be mostly attributed to the increase in variation among some key predictors, like median home value and climate conditions. Restricting yourself to just time series variation of, e.g., home prices within a single county does not really reveal the relationship between land value and home prices in the same way that observing a panel of county-level prices would.  
\end{itemize}

\textit{Our work is policy-relevant}
\begin{itemize}
    \item Broader coverage allows for conservation planning in areas that would otherwise have to rely on estimates in neighboring states
    \item Inclusion of residential real estate market indicators--and validation of how predictive they are of undeveloped land values--could allow for extensibility into areas where real estate information is available but other predictors are either not available or only available at highly aggregated spatial scales.
    \item The importance of climatic variables in our variables indicates that climate change will have an effect on land values, and therefore on conservation planning. An interesting future direction could be quantifying the \textit{causal} effect of each of our climate variables in a more Ricardian-like paper, though I'm not sure what our ``angle" would be, given how ubiquitous that literature is.
    \item FRR models' expansion into rural areas is useful, since that's where high-impact conservation easements/purchasing are happening--but we'd need to confirm that this is true and why. Is it merely a function of lower cost, or is there some higher ecological value in these remote areas?
\end{itemize}

\textit{Our work contributes to the literature}
\begin{itemize}
    \item Confirms Ricardian theory of land attribute capitalization, like soil type and climate conditions.
\end{itemize}

\section{Conclusion}

To effectively preserve and maintain critical habitats and ecosystems, conservation advocates, policymakers, and managers rely on land cost estimates to weigh different strategies in the face of scarce resources. Hindered by limitations of data and computing power, efforts to produce land value estimates have suffered from low spatial resolution. Nolte (2020), approaching the general problem of land value estimation, established a methodology and database that greatly improved value estimates of undeveloped land, employing parcel-level sales data and a large set of predictors. As developers and agricultural producers look to undeveloped or partially agricultural land to convert to lucrative use, conservation planners require a thorough and reliable picture of property valuation in order to cost-effectively build conservation agendas.

In this paper, we improve the accuracy and coverage of previous estimation models for undeveloped land value, leveraging an added set of economically-relevant, high resolution climatic and ecological predictors, as well as incorporating time series data on county-specific residential real estate markets. In addition, we build our models at larger scales, corresponding to regional agricultural markets, which expands the overall reach by 376 counties.

Median home value, climatic information, and several soil type variables contribute substantially to predictive power across county and farm resource region models. Compared to Nolte (2020), our estimates reduce prediction error by 27\% across the conterminous US.
