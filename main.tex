...
To validate the practical benefits of our modeling approach, one might ask whether it facilitates more cost-effective achievement of fixed conservation goals. For instance, the Biden administration has established the ambitious target of conserving 30 percent of American lands by 2030. At least a portion will be secured via financial contracting with private landowners. As a demonstrative exercise, we imagine a single agency is tasked with purchasing 30 percent of the land (in hectares) in our dataset, equipped with a set of land value predictions generated by either a) the true underlying sales value; b) the full predictor farm resource region model; or c) the restricted predictor county model. 

Using each of these approaches, we rank the entire dataset of sales records by ascending predicted price, then purchase each parcel until 30 percent of the total land area (8.4 million hectares) has been conserved and compare the total expenditures (Figure \ref{fig:cost_effective_30by30}). If the agency had access to the true underlying sale values, the endeavor would cost \$6.9 billion. Using the restricted county model to target purchasing would approximately double the cost, to \$13.2 billion. Relative to this, the full resource region model could save the agency \$500 million dollars, securing 30 percent of the land area at a cost of \$12.7 billion. To provide a comparison between modeling approaches that factors out the expanded scope of the resource region model, we also create candidate parcel sets for the true information and full resource region model that only include parcels to which the restricted county model had access (starred columns in Figure \ref{fig:cost_effective_30by30}). In this scenario, the full resource region model and the restricted county model produce nearly identical costs, suggesting the savings arise from the expanded set of predictions from which the agency can select candidates for purchase.  

To contextualize the magnitude of the \$500 million savings, the USDA annually spends just under \$500 million on land easements (e.g., Agricultural Conservation Easement Program) and between \$2 and \$2.5 billion on land retirement (e.g., Conservation Reserve Program).
...
\begin{figure}[H]
    ...
    \caption{Total actual cost of purchasing 30 percent of land area (in hectares) from base dataset. Purchase order is determined by ranking predicted sales price as generated by each model.}
